...
# Threshold used to define successful memorisation/generalisation.
ACCURACY_THRESHOLD = 99.0

# Require two consecutive logged evaluations where BOTH
# train and validation accuracy are >= 99%.
REQUIRED_CONSECUTIVE_SUCCESSES = 2

# Absolute safeguard even if the clock deadline is changed.
...
STAGES = [
    {
        "name": "D1_seed1_50pct_wd1",
        "description": (
            "Seed robustness: division mod 97, "
            "50% training data, weight decay 1, seed 1"
        ),
        "max_steps": 50_000,
        "train_pct": 50,
        "weight_decay": 1.0,
    ...
    {
        "name": "D2_seed2_50pct_wd1",
        "description": (
            "Seed robustness: division mod 97, "
            "50% training data, weight decay 1, seed 2"
        ),
        "max_steps": 50_000,
        "train_pct": 50,
        "weight_decay": 1.0,
    ...
    {
        "name": "D3_fraction40_wd1",
        "description": (
            "Training-fraction calibration: division mod 97, "
            "40% training data, weight decay 1"
        ),
        "max_steps": 100_000,
        "train_pct": 40,
        "weight_decay": 1.0,
    ...
    {
        "name": "D4_fraction60_wd1",
        "description": (
            "Training-fraction calibration: division mod 97, "
            "60% training data, weight decay 1"
        ),
        "max_steps": 50_000,
        "train_pct": 60,
        "weight_decay": 1.0,
    ...
    {
        "name": "D5_fraction70_wd1",
        "description": (
            "Training-fraction calibration: division mod 97, "
            "70% training data, weight decay 1"
        ),
        "max_steps": 40_000,
        "train_pct": 70,
        "weight_decay": 1.0,
    ...
    },
    {
        "name": "D6_50pct_wd03",
        "description": (
            "Weight-decay calibration: division mod 97, "
            "50% training data, weight decay 0.3"
        ),
        "max_steps": 80_000,
        "train_pct": 50,
        "weight_decay": 0.3,
    ...
    },
    {
        ...
        "description": (
            "Weight-decay calibration: division mod 97, "
            "50% training data, weight decay 0.1"
        ),
        "max_steps": 100_000,
        "train_pct": 50,
        "weight_decay": 0.1,
...
def prevent_windows_sleep():
    """
    Prevent Windows from automatically sleeping while experiments run.
    The monitor is still allowed to turn off.
    """
    if os.name == "nt":
...
def calculate_deadline():
    ...
    Stop at whichever comes first:
      1. 8:50 pm today
      2. MAX_SUITE_HOURS after launch
    """
...
def read_raw_metrics(run_dir):
    ...
    try:
        ...
        # Lightning may be updating the file at this exact moment.
        return []


def build_timeline(run_dir):
    """
    Lightning can write different metrics to different rows
    for the same optimisation step.

    Merge them into one row per step.
    """
...
def successful_evaluation_steps(run_dir):
    """
    Return logged evaluation steps where BOTH:
        full_train_acc >= threshold
...
def consecutive_success_count(run_dir):
    """
    Count successful evaluations from the END of the evaluation history.

    This means two old successes separated by a later failure will not
    incorrectly trigger early stopping.
    """
    ...
    for row in reversed(timeline):
        ...
        # Ignore rows that are not full evaluation rows.
        if (
            train_acc is None
            or val_acc is None
        ):
...
def export_analysis_metrics(run_dir):
    """
    Export a clean one-row-per-step CSV for thesis analysis.
    """
...
def calculate_result(
    ...
    # Practical classification for our calibration.
    if (
        train99 is not None
        and val99 is not None
    ):
...
def run_stage(
    ...
    print(
        f"Required consecutive successes: "
            ...
            while (
                ...
                if stage_has_grokked(
                    ...
                    print(
                        ...
                        f"for "
                        f"{REQUIRED_CONSECUTIVE_SUCCESSES} "
                        "consecutive logged evaluations."
                    )
...
def main():
    ...
    print("=" * 90)
    print("GROKKING CALIBRATION SUITE")
    print("=" * 90)
    ...
    print(
        "Each run stops when train AND validation "
        f"reach >= {ACCURACY_THRESHOLD:.0f}% "
        f"for {REQUIRED_CONSECUTIVE_SUCCESSES} "
        "consecutive logged evaluations."
    )
    ...
    print(
        f"Status updates every {POLL_SECONDS} seconds."
    )
